...
//static uint8_t ppsTalkOk = 0;		//告诉定位系统准备开始积分
//static uint8_t ppsReady = 0;		//定位系统准备完毕开始发数

//static Pos_t ppsReturn={0.f};		//默认返回值处理后
Pos_t ppsReturn={0.f};	

void LocatorInit(void)
{
	Serial_SendString(Serial1, "ACT0");
	Delay_ms(10);    
...
void Locator_SerialIsr(void)
{
	...
	/*	发 送 中 断	
	if(USART_GetITStatus(USART1, USART_IT_TC))	//Tx Int
	{
